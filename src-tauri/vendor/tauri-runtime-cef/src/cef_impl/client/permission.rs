    ...
    fn on_request_media_access_permission(
      ...
      match self.decide(requested_permissions, media_permission_kind) {
        ...
        AppDecision::Allowed => {
          ...
          if grants_desktop_capture(requested_permissions) && !GRANT_DISPLAY_CAPTURE.load(std::sync::atomic::Ordering::Relaxed) {
            return 0;
          }
...
static GRANT_DISPLAY_CAPTURE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// Amni patch: let an `Allow` for [`PermissionKind::DisplayCapture`] grant desktop capture.
///
/// Meant for Linux builds using Chromium's PipeWire capturer, where the grant goes to
/// xdg-desktop-portal and the desktop's own dialog asks the user what to share.
pub fn grant_display_capture(enabled: bool) {
  GRANT_DISPLAY_CAPTURE.store(enabled, std::sync::atomic::Ordering::Relaxed);
}
